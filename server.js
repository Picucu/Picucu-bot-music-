...
const commands = {
	...
	'play': (msg) => {
		if (queue[msg.guild.id] === undefined) return msg.channel.sendMessage(`Use ${tokens.prefix}add to tell me what to play!`);
		if (!msg.guild.voiceConnection) return commands.join(msg).then(() => commands.play(msg));
		if (queue[msg.guild.id].playing) return msg.channel.sendMessage("I'm already Playing This!");
		let dispatcher;
		...
		(function play(song) {
			console.log(song);
			if (song === undefined) return msg.channel.sendMessage('No more requests!').then(() => {
	...
	'join': (msg) => {
		return new Promise((resolve, reject) => {
			...
			if (!voiceChannel || voiceChannel.type !== 'voice') return msg.reply('I couldn\'t connect to your voice channel... Sorry');
